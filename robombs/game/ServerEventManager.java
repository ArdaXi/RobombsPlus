...
/**
 * A helper class for the server to ease event processing. An event is send from the client to the server and processed there.
...
public class ServerEventManager {
     ...
     * @param servMan ServerObjectManager the server object manager that lives on this server
     * @param server BlueThunderServer the actual server instance
     * @param ci ClientInfo the client info of the client from which the event came from
     * @return DataContainer[] an optional response. May be null (and usually is in the current implementation).
     */
    ...
    public synchronized DataContainer[] processRoundLost(final Event event, ServerObjectManager servMan, final BlueThunderServer server) {
    	...
    	if (aliveCnt<=1 && !threadRunning) {
			...
			new Thread() {
    			...
    			public void run() {
    				try {
    		    		...
    		    		InfoLine il=new InfoLine(InfoLine.ROUND_COMPLETED ,0 ,name,"");
    		    		dc.add(il);
    		    		server.broadcast(dc);
    					...
    					processLevelRequest(event, server); // Send a "next-level-event"
    					Thread.sleep(300); // Give some time for the client to get the event!
    				} catch(Exception e) {
    		...
    		if (aliveCnt==botCnt && !threadRunning && botCnt!=0) {
    	        ...
    	        Event ev = new Event(Event.ONLY_BOTS_LEFT, null, target); // Actually, the target doesn't matter...
    	        edc.add(ev);
     ...
     * @param event Event the event
     * @param ci ClientInfo the client info
     * @param server BlueThunderServer the server instance
     * @return DataContainer[] an optional response. May be null.
     */
    ...
    private DataContainer[] processExplosionHit(Event event, ServerObjectManager servMan, BlueThunderServer server) {
        ...
        if (target != null) {
        	...
        	// This may happen due to a high latency of a client, that triggers an event on an already removed object.
    ...
    private DataContainer[] processCrateHit(Event event, ServerObjectManager servMan, BlueThunderServer server) {
    	EventDataContainer edc = new EventDataContainer();
    	Event eve = new Event(Event.REMOVE_CRATE, -99, event.getTargetID(), -99);// Crate has been hit
	...
	private void processBomb(LocalObject source, LocalObject target, Event event, BlueThunderServer server)  {
		if (target.getValue()==LocalBombManager.VALUE_ACTIVE || target.getValue()==LocalBombManager.VALUE_DISABLED) {
			EventDataContainer edc = new EventDataContainer();
		    Event eve = new Event(Event.BOMB_HIT, source, target);// Bombe getroffen
		    if (source!=null) {

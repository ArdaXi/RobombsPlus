...
/**
 * An abstract base class for client. There is a visual client (the one that you are using when playing the game)
 * and a bot client. The bot client holds a simplified version of the game world and has no display methods.
 * @author EgonOlsen
 *
 */
public abstract class AbstractClient {
	...
	protected SimpleVector[] spawnPoints = new SimpleVector[] {
			new SimpleVector(60, -10, 100), new SimpleVector(40, -10, 100),
			new SimpleVector(60, -10, 120), new SimpleVector(40, -10, 120) };
	...
	protected void checkWaitingState() {
		if (state.getState()==NetState.STATE_WAITING && waitTimer.getTicks()>0) {
			...
			// This shouldn't be needed anymore as it obviously was a synchronization problem, but one never knows...
	...
	public void beforeReceiving() {
		received = false;
		if (coMan != null) {
			// Erstmal alle als nicht übertragen markieren. Sollten sie es dennoch werden, wird
	...
	/**
	 * Checks for a time out of other clients. If a remote client can't transmit any data, it can't transmit
	 * that it's gone either. This is handled here by removing objects from clients that seem to be lost.
	 * @throws Exception
	 */
	...
	/**
	 * Updates all entities (like player meshes) that are remote, i.e. that came from the server.

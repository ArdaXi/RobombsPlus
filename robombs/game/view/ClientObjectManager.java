...
/**
 * A manager class for handling client objects. This manager holds references to all client objects that this client
 * knows of (even remote ones).
 */
public class ClientObjectManager {
     ...
     * @param co the view object
     * @return LocalObject the underlying local object that the view object represents
     */
    ...
    /**
     * Gets a client object that has been created on a remote client with a given id. If no such
    ...
    public int getPlayerCount() {
    	int cnt=0;
    	for (ClientObject co:objects.values()) {
    		// A player with a very low y-position is actually there, but not placed on the floor.
    ...
    /**
     * Returns the view object for a given local object. If the manager already knows of this object, it returns
     * that one. If it doesn't, it creates a new one and returns that.
     * @param lo the local object on which the view is based
     * @param world the current world (needed, because the manager has to add the new view object to it)
     * @param bulMan the manager for the local bullets (because collisions of local bullets with this object has be determined)
    ...
    public static ClientObject createLocalPlayerObject(LocalObject lo, World world, ClientEventQueue eventQueue) {
    	// Der Player muss nicht vom Manager verwaltet werden. Er wird hier nur erzeugt.

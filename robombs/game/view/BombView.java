...
public class BombView extends ClientObject {
	...
	private long touchedTime=0; // A small timerout value to avoid double touching of bombs
    ...
    public void enterCollisionMode(LocalPlayerObject player) {
    	...
    	if (colMesh!=null && getVisibility() && getBackValue()!=LocalBombManager.VALUE_INACTIVE) {
    		...
    		if (player==null || bombPos.calcSub(player.getPosition()).length()>=LocalBombManager.ACTIVATION_DISTANCE*2) {
    			...
    			// If the player is located inside the bomb, it doesn't block...this happens seldom,
    			// but it happens. Most likely due to latency problems or something. This hack avoids
    ...
    public void process(long ticks, Level level) {
        ...
        if (!pOld.equals(getTransformedCenter())) {
        	...
        	// The bomb has moved! We have to update the bomb mask to reflect this.
    	...
    	switch(val) {
	    	...
	    	case LocalBombManager.VALUE_DISABLED:
	    		...
	    		if (getScale()<=INACTIVE_SCALE) {
	    			// Fix the scale to 1...otherwise, a remote client may still have the inactive scaling, which is bad...
	    			setScale(1f);
	    		}
    			if ((cnt&8)==8) {

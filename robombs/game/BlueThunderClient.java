...
/**
 * The client, i.e. the actual game. This client also opens a window with a simple server browser to join
 * an existing server or start a new one.
 */
public class BlueThunderClient extends AbstractClient implements DataTransferListener,
		ClientPreProcessor, GameClient, SelectionListener, EventProcessor {
	...
	public void connect(ServerEntry server, String name) throws Exception {
		if (!disConnectScheduled) {
			try {
				...
				// Decide if active transfers should be used (LAN) or not (Internet)
				...
				Globals.activeTransfer=false;
				Globals.activeTransferForBots=false;
				NetLogger.log("Client "+getClientID()+": Internet transfer mode forced!");
				*/
	...
	public void begin() {
		// The Colorizer has to be resetted here, because otherwise, it may have created some
		// data between a level request and the actual start of the game. This is not 100% correct
	...
	public void logout(int cid, PlayerInfo pi) {
		if (botClient!=null && botClient.getClientID()==cid) {
			if (botClient.getBotCount()==1) {
				// The last one goes away...terminate that client. That will remove the remaining
	...
	public void shutDownServer() throws Exception {
		if (serverImpl != null) {
			...
			InfoLine il = new InfoLine(InfoLine.SYSTEM_OUT, 0, "msg",
					"Server stopped!");
	...
	/**
	 * Starts a single player game. That's a game with all maps in the rotation plus 3 bots.
	 */
	...
	protected void processInfo(InfoLine il) {
		...
		if (il.getType() == InfoLine.ROUND_COMPLETED) {
			...
			roundMessage = new Label((int) (buffer.getOutputWidth()/2-buffer.getOutputWidth()/13.3333f-dim), (int)(buffer.getOutputHeight()/2+buffer.getOutputWidth()/10.66666f)-bannerFont.fontHeight/2);
			roundMessage.setFont(bannerFont);
			roundMessage.setText(name);
			roundMessage.setColor(Color.BLACK);

			if (player.isDead() && player.getSpecialValue()<=0) {
				// Obviously no win
		...
		if (il.getType() == InfoLine.READY_YOURSELF && !isReady()
				&& !runsServer()) {
	...
	public void processEvent(Event event) {
		...
		case Event.TAUNT:
			// event.getOrigin() ist oftmals zu leise...daher nur aus der Richtung aber laut!
			...
			if (!roundCompleted) {
				...
				if (event.getTargetClientID() != clientImpl.getClientID()) {
					...
					if (player.getSpecialValue()<=0) {
						...
						SoundManager.getInstance().play("boo", player.getPosition());
						// processEvent is already synchronized to world...no need to resync it here...
		...
		case Event.BOMB_TOUCHED:
			if (player.isInvincible() && event.getSourceClientID()==clientImpl.getClientID() && !player.getPlayerPowers().canKick()) {
				...
				if (event.getTargetClientID() == clientImpl.getClientID() /*&& player.getPlayerPowers().canKick()*/) {
					// Created by myself
		...
		case Event.COLLECT_ITEM:
			...
			SoundManager.getInstance().play(sound, event.getOrigin()); // Play sound...
		...
		case Event.LOGIN_REJECTED:
			if (event.getTargetClientID() == clientImpl.getClientID()) {
				if (event.getSourceID() == -99) {
					serverSel
							.message("Login rejected - the game is already running!");
				}
		...
		case Event.FIRE:
			if (event.getSourceClientID() != clientImpl.getClientID()) {
				...
				if (fireTicker.getTicks()>0) {
					// This isn't really correct...but making it correct is very expensive. So we don't care for now...
		...
		case Event.ENTITY_REMOVE:
			// For "normal" Objects, the server just doesn't transmit a removed object. For bombs,
			// this isn't sufficient, because they have to be removed from the manager's grid mask
			// This is why the server broadcasts this removal. There is no check, if this is really a bomb.
			// The server doesn't broadcast anything else ATM, so it's hacky but fine this way. If this
	...
	private void initBuffer() {
		...
		buffer = new FrameBuffer(width, height, this.antiAliasing*10); // Relies on the constants...:-)
...
/*
		if (!buffer.isInitialized()) {
			// Error with this mode? Try 1024*768 instead...this has to be replaced by a "real" video mode selection
			// in a "real" game.
...
*/
	...
	private void initPlayerView() {
		...
		playerView.setCollisionMode(Object3D.COLLISION_CHECK_SELF
				| Object3D.COLLISION_CHECK_OTHERS);
	...
	private void fire(long ticks) {
		if (!serverSel.isVisible() && !creditMode && state.getState()==NetState.STATE_RUNNING && allPlayersHaveSpawned()) {
			...
			if ((KeyStates.fire || mouse.buttonDown(0) || mouse.buttonDown(1) || sick==PlayerPowers.DROP_IMMEDIATELY)
					&& clientImpl != null && clientImpl.isConnected()) {
			...
			if (sick==PlayerPowers.DROP_IMMEDIATELY) {
				// In this mode, nothing has to be released...you'll drop the bomb anyway...:-)
				hasToRelease=false;
			}
	...
	/*
		if (fireTicker.getTicks() > 0) {
			...
			LocalObject bul = bulMan.addBullet(player, world, eventQueue);
					...
					synchronized (SYNC) {
						...
						do {
							SimpleVector s = new SimpleVector(
									spawnPoints[(clientImpl.getClientID()-1 + its)
											% spawnPoints.length]);
							SimpleVector ss = new SimpleVector();
							ss.y=-3;

							if ((isFree(s,level.getCrateManager(), bombMan, BlueThunderClient.this.coMan) && world.checkCollisionSpherical(s, ss, 3f).equals(ss))
									|| its > 15) {
								respawn = s;
							} else {
								its++;
						...
						// Align the player, so that he looks into the level's center!
						...
						synchronized(world) {
							// Has to be synchronized with rendering, because the ShadowHelper changes the
							// world's camera.
	 ...
	 * @param ticks long the number of ticks passed since last call
	 */
	...
	/**
	 * Draw the GUI (i.e. the server selection as there is no more GUI in this example) if visible.
	 */
	private void drawGUI() {
		if (serverSel.isVisible()) {
			...
			mouse.hide();
			if (creditMode) {
				buffer.blit(TextureManager.getInstance().getTexture("credits"), 0, 0, buffer.getOutputWidth()/2-height/2, buffer.getOutputHeight()/2-height/2, 512, 512, height, height, 60, false, null);
	...
	private void taunt() {
		int taunt=-1;
		if (KeyStates.one) {taunt=0;}
		if (KeyStates.two) {taunt=1;}
		if (KeyStates.three) {taunt=2;}
		if (KeyStates.four) {taunt=3;}
		if (KeyStates.five) {taunt=4;}
		if (KeyStates.six) {taunt=5;}
		if (KeyStates.seven) {taunt=6;}
		if (KeyStates.eight) {taunt=7;}
		if (KeyStates.nine) {taunt=8;}
		if (KeyStates.ten) {taunt=9;}
	...
	private void gameLoop() throws Exception {
		...
		while (!quit) {
			...
			if (throttling) {
				...
				Thread.sleep(sleepTime); // Gives network and other threads some time to breath
	...
	private void setSun(Projector proj) {
		...
		light.setIntensity(new SimpleVector(100,100,100));
	}
	...
	private void exit() throws Exception {
		...
		Thread.sleep(200);
		//buffer.disableRenderer(IRenderer.RENDERER_OPENGL); // Causes a crash on some machines...
		buffer.dispose();
	...
	private void updatePlayer(long ticks) {
		if (state.getState() == NetState.STATE_RUNNING  && allPlayersHaveSpawned()) {
			...
			//System.out.println("Player at :"+level.getMask().getGrid(player.getPosition().x, player.getPosition().z));
